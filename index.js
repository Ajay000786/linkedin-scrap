...
async function scrapeLinkedIn(companyList) {
  ...
  if (isLoginPage) {
    await page.type('#username', 'nimcetajay7@gmail.com');
    await page.type('#password', 'Ajay@7999');
    await page.click('.login__form_action_container button');
  
    await page.waitForNavigation();
  }

  const Writeoncsv = createObjectCsvWriter({
    path: 'results.csv',
    header: [
...
for (const pageUrl of comanyLinkedinUrl) {
  ...
  console.log(data);
  await Writeoncsv.writeRecords(data);
